...
import { formatMonitorLabel, formatRelativeTime, formatSurfaceLabel, formatValue } from '../../../utils/runtime-formatters.js';
import { RuntimeMetric } from '../RuntimeMetric.jsx';

export function HomeSection({ runtime, runtimeHub, user, refreshing, onRuntimeRefresh }) {
          ...
          <div className="desktop-HubList">
            {crossSurfaceActivity.slice(0, 8).map((entry) => (
              <div key={entry.eventId} className="desktop-HubListItem">
                ...
                <span className="desktop-HubMeta">{formatRelativeTime(entry.occurredAt)}</span>
              </div>
            ))}
          </div>
        </Card>
