...
const baseRuntimeSchema = defineSchema({
  ...
  PUBLIC_APP_NAME: rules.optionalString("$TANDARD / DEVIANT$"),
  PUBLIC_PRODUCT_NAME: rules.optionalString("$TANDARD / DEVIANT$")
});
...
const webRuntimeSchema = defineSchema({
  ...
  NEXT_PUBLIC_APP_NAME: rules.optionalString("$TANDARD / DEVIANT$"),
  NEXT_PUBLIC_PRODUCT_NAME: rules.optionalString("$TANDARD / DEVIANT$")
});
...
const mobileRuntimeSchema = defineSchema({
  ...
  EXPO_PUBLIC_APP_NAME: rules.optionalString("$TANDARD / DEVIANT$"),
  EXPO_PUBLIC_PRODUCT_NAME: rules.optionalString("$TANDARD / DEVIANT$"),
...
const extensionRuntimeSchema = defineSchema({
  ...
  EXTENSION_APP_NAME: rules.optionalString("$TANDARD / DEVIANT$")
});

const electronRuntimeSchema = defineSchema({
  ELECTRON_API_BASE_URL: rules.optionalString("http://localhost:4000"),
  ELECTRON_APP_NAME: rules.optionalString("$TANDARD / DEVIANT$")
});
